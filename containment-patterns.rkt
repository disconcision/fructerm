...
#|
   ...
   ⋱ enables concise updates on highly nested s-expressions

   this library implements several match-expanders which
   can be required seperately and used with racket/match

   the pattern (⋱ <context-name> <pattern>) matches some
   <target> and binds a procedure to <context-name> and a
   list of <matches> to <pattern> satisfyng:

      (equal? (apply <context> <matches>) target)

   containment patterns were created for syntax rewriting
   in structured editing and algebraic stepper prototypes

   ⋱ captures a single-holed context and the first match
   hit. ⋱1 insists that the hit is unique. ⋱+ captures a
   list of matches and their multi-holed context

   an n-holed context is captured as a normal n-ary procedure
   which can be reconstituted in the match template by
   either normal function application or using ⋱ for symmetry


 
   LIBRARY STABILITY 2018.10

   the above content is pretty stable but the below-explained
   capture-when and until options are still under consideration
